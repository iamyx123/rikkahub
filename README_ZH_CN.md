...
- **图片预览一键打印**：图片预览页在「下载」旁新增「打印」按钮，任意图片（含拍照截取的老师课件）可直接从喵喵机打印，省去导回官方 App 的繁琐。
- **AI 回复一键打印**：每条 AI 回复的「更多」里新增「打印这条回复」，将含公式的回答**原生渲染为图片**后紧凑黑白打印（公式清晰）。
- **打印机体验**：蓝牙自动扫描、点击连接、断线自动重连；自动检测纸张宽度（2 寸 576px / 3 寸 864px）；黑白（文字/题目）与灰度（照片）两种打印；浓度可调。
