...
class McpManager(
    ...
    suspend fun callTool(serverId: Uuid, toolName: String, args: JsonObject): List<UIMessagePart> {
        ...
        return result.content.map { content ->
            when (content) {
                is TextContent -> UIMessagePart.Text(content.text).withMcpAudience(content.annotations)
                is ImageContent -> convertImageContentToFilePart(content).withMcpAudience(content.annotations)
